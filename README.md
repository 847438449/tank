...
已新增蓝牙耳机稳定模式，重点针对 Windows 蓝牙播放时的 loopback discontinuity：

- 采集层固定 48000 Hz，后续在预处理阶段重采样到 16000 给 whisper。
- 蓝牙模式默认使用更大 blocksize（4096，可在 `config.py` 调整）。
- 使用更长 ring buffer（默认 1024+ 帧）保证录音线程不被识别线程阻塞。
- 每次启动都会重新枚举并绑定当前默认扬声器设备（不复用旧实例）。
- 若设备名出现 `Hands-Free / Headset / 免提`，启动时给出警告。
- 对短暂 discontinuity 自动插入静音帧并做短块补齐，避免整段报废。
- GUI 刷新频率已下调（300ms）以减轻主线程压力。
